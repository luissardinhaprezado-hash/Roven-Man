...
export default function ProductsManager() {
  ...
  const save = async () => {
    ...
    if (form.originalPrice.trim()) {
      originalPrice = parseFloat(String(form.originalPrice).replace(",", "."))
    }
    let shipping = 0
    if (form.shipping.trim()) {
      shipping = parseFloat(String(form.shipping).replace(",", "."))
    }
    const data = {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          <button type="button" disabled={busy} onClick={publish} className="border border-zinc-600 px-4 py-2 text-sm rounded">
            {busy ? "..." : "Publicar no site"}
          </button>
          <button type="button" onClick={openNew} className="bg-amber-500 text-black px-4 py-2 text-sm font-semibold rounded flex items-center gap-2">
      ...
      {showForm && (
        <div className="border border-zinc-800 rounded-lg p-4 space-y-3 bg-zinc-950">
          <div className="flex justify-between">
            <h3 className="font-semibold">{editingId ? "Editar" : "Novo"}</h3>
          ...
          <p className="text-xs text-zinc-400">Foto da galeria</p>
          <input type="file" accept="image/*" onChange={onFile} className="text-sm text-zinc-400 w-full" />
          {form.image ? <img src={form.image} alt="" className="w-20 h-28 object-cover rounded" /> : null}
          <input value={form.image.indexOf("data:") === 0 ? "" : form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} placeholder="Ou link https da foto" className="w-full bg-zinc-900 border border-zinc-700 px-3 py-2 rounded text-sm" />
          <button type="button" onClick={save} className="bg-amber-500 text-black px-6 py-2 font-semibold text-sm rounded">
            Guardar e publicar
          </button>
        </div>
      )}
        ...
        {items.map((p) => (
          <div key={p.id} className="flex gap-3 border border-zinc-800 p-3 rounded items-center">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-zinc-400">EUR {Number(p.price).toFixed(2)}</p>
            </div>
            <button type="button" onClick={() => openEdit(p)} className="p-2">
              <Pencil size={16} />
            </button>
            <button
              type="button"
